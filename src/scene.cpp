...
bool loadMesh(Geometry& result, const char* path, bool buildMeshlets, bool fast)
{
...
bool loadScene(Geometry& geometry, std::vector<Material>& materials, std::vector<MeshDraw>& draws, std::vector<std::string>& texturePaths, std::vector<Animation>& animations, Camera& camera, vec3& sunDirection, const char* path, bool buildMeshlets, glm::vec3& euler, bool fast)
{
	clock_t timer = clock();
